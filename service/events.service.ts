  ...
  events.forEach((event) => {
    if (event.isFeatured) {
      ...
      for (let i = 0; i < MONTH_VIEW_COLORS_LENGTH; i++) {
        ...
        const isColorAvailable = eventDates.every((date) => !dayColorAssignments[date]?.has(potentialColor));
      ...
      assignedColor = assignedColor || `color${colorIndex % MONTH_VIEW_COLORS_LENGTH}`;
...
export const getAllEvents = async () => {
    ...
    let formattedEvents = allEvents?.map((event: any, index: number) => {
      let dayDifference = differenceInDays(event.start_date, event.end_date, event.timezone);
      ...
      return {
        ...
        dateRange: formatDateForSchedule(event.start_date, event.end_date, event.timezone),
  ...
  export const getRefreshedAgenda = async (eventId: string) => {
    const result = await fetch(`${process.env.EVENT_AGENDA_REFRESH_URL}/api/event/${eventId}`, {
