...
export const getAllEvents = async (location: any, year?: number) => {
  ...
  let formattedEvents = filteredEvents?.map((event: any, index: number) => {
    ...
    return {
      ...
      latitude: event.address_info?.latitude ? parseFloat(event.address_info.latitude) : null,
      longitude: event.address_info?.longitude ? parseFloat(event.address_info.longitude) : null,
      city: event.address_info?.city ?? "",
      state: event.address_info?.state ?? "",
